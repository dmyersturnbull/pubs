...
# using 'Intensity Pynd Alkko WCl'
# normalize by fraction of total intesity
sum_col = df['Intensity Pynd_AlkKO_WCL'].sum()
df['norm'] = df['Intensity Pynd_AlkKO_WCL'].div(sum_col)

# TODO later
# make new Epsilon? column, where epsilon = (10^5)/[total intensity]
# if intensity value = 0:
#    df.replace({'Intensity Pynd_AlkKO_WCL' : { '0' : 10000}})
#    df['Epsilon'] = df
